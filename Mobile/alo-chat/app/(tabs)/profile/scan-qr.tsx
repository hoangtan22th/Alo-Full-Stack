...
export default function QrScannerScreen() {
  ...
  const handleBarCodeScanned = async ({ type, data }: { type: string; data: string }) => {
    ...
    if (!data.startsWith("alo-chat://login?token=")) {
      Alert.alert("Lỗi", "Mã QR không hợp lệ, vui lòng quét lại mã đăng nhập của Alo-Chat.", [
    ...
    try {
      ...
      console.error(error);
      const msg = error.response?.data?.message || "Đã có lỗi xảy ra hoặc mã QR đã hết hạn.";
  ...
  if (hasPermission === false) {
    return (
      <View className="flex-1 bg-white items-center justify-center px-6">
        <Ionicons name="camera-outline" size={64} color="#ccc" className="mb-4" />
        <Text className="text-center text-lg mb-6">Xin lỗi, chúng tôi cần quyền truy cập camera để quét mã QR.</Text>
        <TouchableOpacity 
  ...
  return (
    <View className="flex-1 bg-black">
      <View style={{ paddingTop: insets.top, paddingHorizontal: 16, zIndex: 10 }} className="flex-row items-center py-4">
        <TouchableOpacity onPress={() => router.back()} className="w-10 h-10 bg-white/20 rounded-full items-center justify-center">
          <Ionicons name="close" size={24} color="white" />
        </TouchableOpacity>
        <Text className="text-white flex-1 text-center text-lg font-bold mr-10">Quét mã QR trên Web</Text>
      </View>
        ...
        <View style={StyleSheet.absoluteFillObject} className="bg-black/50">
          ...
          <View className="flex-1 items-center pt-8">
            <Text className="text-white/80 text-center px-10 font-medium">
              Di chuyển camera đến khu vực mã QR trên màn hình máy tính để đăng nhập.
            </Text>
          </View>
        </View>
      </CameraView>
