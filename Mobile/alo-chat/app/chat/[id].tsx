...
export default function GlobalChatScreen() {
  ...
  const panGesture = Gesture.Pan()
    ...
    .onUpdate((event) => {
      ...
      translateY.value = Math.max(SNAP_POINTS.TOP, context.value.y + event.translationY);
    })
    .onEnd((event) => {
  ...
  const handleRevoke = async () => {
    ...
    if (ok) {
      setMessages((prev) =>
        prev.map((m) => (m._id === msgId ? { ...m, isRevoked: true, content: "" } : m))
      );
    }
  };
  ...
  useEffect(() => {
    if (!reactionDetailMsgId || !reactionDetailMsg?.reactions) return;

    const fetchMissingUsers = async () => {
      const uniqueUserIds = Array.from(
        new Set(reactionDetailMsg.reactions!.map((r: any) => r.userId))
      );
      
      const missingIds = uniqueUserIds.filter(id => !userCache[id]);
      ...
      await Promise.all(
        missingIds.map(async (id) => {
          try {
            ...
            console.error("Lỗi fetch user in reactions:", id, err);
          }
        })
      );
    ...
    fetchMissingUsers();
  }, [reactionDetailMsgId, reactionDetailMsg?.reactions]);
    ...
    const resolve = async () => {
      ...
      if (needsResolution) {
        try {
          ...
          console.error("Không thể phân giải hội thoại, dùng ID mặc định:", err);
          setResolvedConversationId(initialId);
        }
      } else {
        setResolvedConversationId(initialId);
  ...
  useEffect(() => {
    ...
    const fetchMsgs = async () => {
      ...
      try {
        const msgs = await messageService.getMessageHistory(resolvedConversationId, 50, 0);
        setMessages(msgs);
              ...
              return (
                ...
                >
                  {group.messages.map((msg, idx) => {
                    ...
                    return (
                      <View key={msg._id} className={isLast ? "" : "mb-1"}>
                        <TouchableOpacity
                          ref={(r) => {
                            ...
                            >
                              <View className="flex-row items-center">
                                {Array.from(new Set(msg.reactions.map((r: any) => r.emoji))).map(
                                  (emojiKey: any) => (
                                    <Text key={emojiKey} className="text-[11px] mr-0.5">
                                      {EMOJI_MAP[emojiKey] || "👍"}
                                    </Text>
                                  )
                                )}
                ...
                <View
                  style={{
                    ...
                    top: isTopHalf 
                      ...
                      : Math.max(20, selectedMsgLayout.y - (selectedMsg.senderId === currentUserId ? 240 : 180)),
                    alignItems: selectedMsg.senderId === currentUserId ? "flex-end" : "flex-start",
                  }}
                >
                  {/* Khay Emoji */}
                  <View
                  ...
                  >
                    {!selectedMsg.isRevoked && selectedMsg.type === "text" && (
                      ...
                      >
                        ...
                        <Text className="text-[14px] font-medium text-gray-700">Sao chép</Text>
                      </TouchableOpacity>
                    )}
                    ...
                    <TouchableOpacity className="flex-row items-center gap-3 px-4 py-3 border-b border-gray-50 active:bg-gray-50">
                      ...
                      <Text className="text-[14px] font-medium text-gray-700">Ghim tin nhắn</Text>
                    </TouchableOpacity>

                    {selectedMsg.senderId === currentUserId && !selectedMsg.isRevoked && (
                      <TouchableOpacity 
                      ...
                      >
                        ...
                        <Text className="text-[14px] font-medium text-orange-500">Thu hồi</Text>
                      </TouchableOpacity>
                    )}
                    ...
                    >
                      ...
                      <Text className="text-[14px] font-medium text-red-500">Xóa tin nhắn</Text>
                    </TouchableOpacity>
                  </View>
                </View>
              </View>
      ...
      >
        <GestureHandlerRootView style={{ flex: 1 }}>
          <View className="flex-1 justify-end">
            <Animated.View 
              style={[{ position: "absolute", top: 0, left: 0, right: 0, bottom: 0 }, animatedBackdropStyle]}
              //@ts-ignore
              onStartShouldSetResponder={() => true}
              //@ts-ignore
            ...
            >
              ...
              <View className="flex-1 bg-white">
                <Text className="text-center text-lg font-bold text-gray-900 mt-2 mb-2">Biểu tượng cảm xúc</Text>
                    ...
                    {reactionTabs.map((tab, index) => (
                      <TouchableOpacity
                        ...
                        onPress={() => {
                          ...
                          reactionScrollViewRef.current?.scrollTo({ x: index * Dimensions.get("window").width, animated: true });
                        }}
                        className={`px-4 py-3 items-center border-b-2 ${
                          activeReactionTabIdx === index ? "border-black" : "border-transparent"
                        }`}
                      >
                        <Text className={`text-[15px] ${activeReactionTabIdx === index ? "font-bold text-black" : "text-gray-500"}`}>
                          {tab.label} {tab.key === 'all' ? "" : (reactionDetailMsg?.reactions ?? []).filter((r: any) => r.emoji === tab.key).length}
                        </Text>
                      </TouchableOpacity>
                    ))}
                  </ScrollView>
                ...
                <ScrollView
                  ...
                  onMomentumScrollEnd={(e) => {
                    const newIdx = Math.round(e.nativeEvent.contentOffset.x / Dimensions.get("window").width);
                    ...
                    >
                      <ScrollView className="flex-1 px-4 py-2">
                        {tab.data.map((item: any, idx: number) => {
                          const cached = userCache[item.userId || item.senderId];
                          const displayName = cached?.fullName || item.fullName || "Người dùng";
                          const avatarUri = cached?.avatar || item.avatarUrl || item.avatar || "https://i.pravatar.cc/150";
                          ...
                          return (
                            <View key={idx} className="flex-row items-center py-3">
                              ...
                              <View className="ml-3 flex-1">
                                ...
                                {tab.key === 'all' && (
                                  <View className="flex-row mt-0.5">
                                    {Array.from(item.emojis || []).map((e: any) => (
                              ...
                              {tab.key === 'all' && item.total > 0 && (
                                <Text className="text-gray-400 font-medium">{item.total}</Text>
                              )}
                            </View>
                          );
                        })}
