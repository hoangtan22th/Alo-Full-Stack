...
export const PollMessagePreview = ({ pollId, isSender }: PollMessagePreviewProps) => {
  ...
  if (!poll) {
    return (
      <View style={{ padding: 16, width: 260 }}>
        <Text style={{ color: isSender ? "white" : "#ef4444", fontStyle: "italic" }}>
  ...
  return (
    <View style={{ width: 300, backgroundColor: "white", borderRadius: 16, overflow: "hidden", borderWidth: 1, borderColor: "#e5e7eb", shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.1, shadowRadius: 4, elevation: 3 }}>
      {/* Header */}
      <View style={{ paddingHorizontal: 16, paddingTop: 16, paddingBottom: 12, borderBottomWidth: 1, borderBottomColor: "#f3f4f6", flexDirection: "row", alignItems: "flex-start" }}>
        <View style={{ backgroundColor: "#eff6ff", padding: 6, borderRadius: 8, marginRight: 10 }}>
        ...
        <View style={{ flex: 1 }}>
          ...
          {isPollEnded && (
             <Text style={{ fontSize: 12, color: "#ef4444", fontWeight: "500" }}>Đã kết thúc</Text>
          )}
      ...
      <View style={{ padding: 12 }}>
        {poll.options.slice(0, 5).map((opt) => {
          ...
          const percentage = Math.round((count / (results.reduce((acc, r) => acc + r.count, 0) || 1)) * 100);
          ...
          return (
            ...
            >
              <View style={{ flexDirection: "row", alignItems: "center", marginBottom: 4, paddingHorizontal: 4 }}>
                 {isSelected ? (
                    <CheckCircleIcon size={20} color="#3b82f6" style={{ marginRight: 8 }} />
                 ) : (
                    <View style={{ width: 20, height: 20, borderRadius: poll.settings.allowMultipleAnswers ? 4 : 10, borderWidth: 1.5, borderColor: "#d1d5db", marginRight: 8 }} />
                 )}
                 <Text style={{ flex: 1, fontSize: 14, color: "#374151" }} numberOfLines={1}>
                   {opt.text}
                 </Text>
                 {(!poll.settings.hideResultsUntilVoted || hasVoted || isPollEnded) && (
              ...
              {/* Progress bar */}
              {(!poll.settings.hideResultsUntilVoted || hasVoted || isPollEnded) && (
                <View style={{ height: 4, backgroundColor: "#f3f4f6", borderRadius: 2, overflow: "hidden", marginLeft: 32 }}>
        ...
        {poll.options.length > 5 && (
           <Text style={{ textAlign: "center", fontSize: 12, color: "#6b7280", fontStyle: "italic", marginTop: 4 }}>
      ...
      {/* Actions */}
       <View style={{ flexDirection: "row", borderTopWidth: 1, borderTopColor: "#f3f4f6" }}>
         <TouchableOpacity 
           ...
           style={{ flex: 1, paddingVertical: 12, alignItems: "center", borderRightWidth: 1, borderRightColor: "#f3f4f6" }}
         >
            <Text style={{ fontSize: 14, fontWeight: "600", color: "#6b7280" }}>Chi tiết</Text>
         </TouchableOpacity>
         
         <TouchableOpacity 
           onPress={handleVote} 
           disabled={submitting || readonly || selectedOptions.length === 0}
           style={{ flex: 1, paddingVertical: 12, alignItems: "center", backgroundColor: !hasVoted && selectedOptions.length > 0 && !readonly ? "#3b82f6" : "transparent" }}
         >
            {submitting ? (
               <ActivityIndicator size="small" color={!hasVoted && selectedOptions.length > 0 && !readonly ? "white" : (isSender ? "white" : "#3b82f6")} />
            ) : (
               <Text style={{ 
                 fontSize: 14, 
                 fontWeight: "bold", 
                 color: hasVoted ? "#10b981" : (selectedOptions.length > 0 && !readonly ? "white" : (isSender ? "#d1d5db" : "#9ca3af")) 
               }}>
