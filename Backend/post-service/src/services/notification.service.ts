...
export class NotificationService {
   ...
   */
  public async getNotifications(userId: string, limit: number = 20, skip: number = 0): Promise<INotification[]> {
  ...
  public async markAsRead(notificationId: string, userId: string): Promise<INotification | null> {
    return await Notification.findOneAndUpdate(
      { _id: new Types.ObjectId(notificationId), recipientId: userId },
      { $set: { isRead: true } },
      { new: true }
    );
  }
