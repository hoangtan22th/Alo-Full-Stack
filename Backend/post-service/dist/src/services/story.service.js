...
export class StoryService {
     ...
     * Tạo Story mới
     */
    async createStory(userId, file, caption, privacy = 'FRIENDS_ONLY', music, allowedUsers, blockedUsers, authHeader, duration) {
        if (!file) {
        ...
        const filteredStories = stories.filter((story) => {
            ...
            if (story.privacy === 'CUSTOM') {
                if (story.blockedUsers.includes(userId))
                    return false;
                if (story.allowedUsers.length > 0)
    ...
    async viewStory(storyId, viewerId) {
        ...
        const alreadyViewed = story.viewers.some((v) => v.userId.toString().toLowerCase() === viewerId.toString().toLowerCase());
        if (!alreadyViewed) {
