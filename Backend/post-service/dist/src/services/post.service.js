...
export class PostService {
     ...
     */
    async getFriendIds(userId, authHeader) {
        try {
     ...
     */
    async isFriend(userId, targetUserId, authHeader) {
     ...
     * Tạo bài viết mới
     */
    async createPost(userId, content, files, privacy = 'FRIENDS_ONLY', tags, allowedUsers, blockedUsers, authHeader, backgroundTemplate) {
     ...
     * Chỉnh sửa bài viết
     */
    async editPost(postId, userId, content, privacy, files, keepMediaUrls, // Danh sách các media cũ muốn giữ lại
    tags, allowedUsers, blockedUsers) {
     ...
     * Xóa bài viết
     */
    async deletePost(postId, userId, authHeader) {
        if (!isValidObjectId(postId)) {
            throw new Error('postId không hợp lệ');
        }
        const post = await Post.findById(postId);
     ...
     */
    async getPostDetails(postId, currentUserId, authHeader) {
        if (!isValidObjectId(postId)) {
            throw new Error('postId không hợp lệ');
        }
        const post = await Post.findById(postId);
     ...
     */
    async getHomeFeed(userId, limit = 10, skip = 0, authHeader) {
        const friendIds = await this.getFriendIds(userId, authHeader);
        // Truy vấn bài viết:
