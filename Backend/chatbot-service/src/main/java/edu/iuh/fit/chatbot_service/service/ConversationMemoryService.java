//package edu.iuh.fit.chatbot_service.service;
//
//import org.springframework.ai.chat.messages.Message;
//import org.springframework.ai.chat.messages.UserMessage;
//import org.springframework.ai.chat.messages.AssistantMessage;
//import org.springframework.stereotype.Service;
//import java.util.*;
//import java.util.concurrent.ConcurrentHashMap;
//
//@Service
//public class ConversationMemoryService {
//
//    // Lưu lịch sử hội thoại (dạng Message)
//    private final Map<String, List<Message>> conversationHistory = new ConcurrentHashMap<>();
//
//    // Lưu các thông tin "ghi nhớ" dạng text cho MemoryTool
//    private final Map<String, List<String>> memoryStore = new ConcurrentHashMap<>();
//
//    // Thêm tin nhắn vào lịch sử
//    public void addToHistory(String conversationId, Message message) {
//        List<Message> history = conversationHistory.computeIfAbsent(conversationId, k -> new ArrayList<>());
//        history.add(message);
//        // Giới hạn chỉ giữ 20 tin nhắn gần nhất để tránh quá dài
//        while (history.size() > 20) {
//            history.remove(0);
//        }
//    }
//
//    // Lấy toàn bộ lịch sử (đã giới hạn)
//    public List<Message> getConversationHistory(String conversationId) {
//        return conversationHistory.getOrDefault(conversationId, Collections.emptyList());
//    }
//
//    // Xóa lịch sử
//    public void clearHistory(String conversationId) {
//        conversationHistory.remove(conversationId);
//    }
//
//    // === Các method dành cho MemoryTool (ghi nhớ thông tin) ===
//    public void save(String conversationId, String info) {
//        memoryStore.computeIfAbsent(conversationId, k -> new ArrayList<>()).add(info);
//    }
//
//    public List<String> get(String conversationId) {
//        return memoryStore.getOrDefault(conversationId, Collections.emptyList());
//    }
//
//    public void clearMemory(String conversationId) {
//        memoryStore.remove(conversationId);
//    }
//}