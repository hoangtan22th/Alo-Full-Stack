...
const GROUP_SERVICE_URL =
  process.env.GROUP_SERVICE_URL || "http://localhost:8082/api/v1/groups";
// const USER_SERVICE_INTERNAL_URL = 'http://localhost:8086/api/v1/admin/users';
const USER_SERVICE_INTERNAL_URL =
  process.env.USER_SERVICE_INTERNAL_URL ||
  "http://user-service:8086/api/v1/admin/users";
...
async function getAllUserIds(): Promise<string[]> {
  try {
    console.log(
      `[BroadcastWorker] Fetching all user IDs from ${USER_SERVICE_INTERNAL_URL}/internal/ids`,
    );
 ...
 */
export async function processPersistentBroadcast(
  campaignId: string,
  broadcastData: {
    title: string;
    content: string;
  ...
  try {
    ...
    for (let i = 0; i < userIds.length; i += BATCH_SIZE) {
      ...
      for (let j = 0; j < batchUserIds.length; j += CONCURRENCY_LIMIT) {
        ...
        await Promise.all(
          subChunk.map(async (userId) => {
            try {
              const convoRes = await axios.post(
                `${GROUP_SERVICE_URL}/direct`,
                {
                  targetUserId: userId,
                },
                {
      ...
      const messageDocs = Array.from(userToConvo.entries()).map(
      ...
      console.log(
        `[BroadcastWorker] Campaign ${campaignId}: Processed ${totalProcessed}/${userIds.length} users.`,
      );
    }
    ...
    console.log(
      `[BroadcastWorker] Campaign ${campaignId} finished successfully. Total: ${totalProcessed}`,
    );
  } catch (error) {
    console.error(
      `[BroadcastWorker] Fatal error for Campaign ${campaignId}:`,
      error,
    );
