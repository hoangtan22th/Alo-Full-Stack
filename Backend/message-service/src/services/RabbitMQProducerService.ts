...
// Chỉ giữ lại một class duy nhất
export class RabbitMQProducerService {
  /**
   * Đồng bộ sự kiện ghim/bỏ ghim tin nhắn tới Realtime Service
   */
  async publishMessagePinEvent(data: {
    messageId: string;
    ...
    try {
      await this.publishToRealtimeService("message-pinned", {
      ...
      console.log(
        `[RabbitMQProducer] Message pin event published: ${data.messageId} - ${data.isPinned}`,
      );
    } catch (error) {
      console.error(
        "[RabbitMQProducer] Failed to publish message pin event:",
        error,
      );
    }
  }
  ...
  async publishMessageEvent(messageData: MessageEvent): Promise<void> {
    try {
      ...
      console.log(
        `[RabbitMQProducer] Message event published: ${messageData._id}`,
      );
    } catch (error) {
      console.error(
   ...
   */
  private async publishToRealtimeService(
    event: string,
    payload: any,
  ): Promise<void> {
    try {
      ...
      console.log(
        `[RabbitMQProducer] Debug: Sending to realtime_events queue for room_${payload.room}`,
      );
      const ch = getChannel();
      ch.sendToQueue(
        "realtime_events",
      ...
      console.error(
        "[RabbitMQProducer] Failed to publish to realtime queue:",
        error,
      );
    }
  }
  ...
  async publishMessageReadEvent(readEvent: MessageReadEvent): Promise<void> {
    try {
      ...
      console.log(
        `[RabbitMQProducer] Message read event published to Realtime`,
      );
    } catch (error) {
      console.error(
