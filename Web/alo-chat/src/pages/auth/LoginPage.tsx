...
const LoginPage = () => {
  ...
  const handleSendForgotOtp = async (e: React.FormEvent) => {
    ...
    setError("");
    try {
      await axiosClient.post("/auth/forgot-password/send-otp", { email: forgotEmail });
  ...
  const handleResetPassword = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!otp || !newPassword) return setError("Vui lòng nhập đầy đủ OTP và mật khẩu mới");
    
    setIsSubmitting(true);
    setError("");
    try {
      // Giả định backend nhận { email, otp, newPassword } cho API reset
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#F3F3F4] p-4 font-sans">
      <div className="bg-white border border-gray-100 rounded-[40px] shadow-2xl max-w-5xl w-full p-8 md:p-16 grid grid-cols-1 md:grid-cols-2 gap-12 items-center min-h-[600px]">
          ...
          {view === "LOGIN" && (
            <div className="animate-in fade-in slide-in-from-left-4 duration-300">
              ...
              <form onSubmit={handleLogin} className="space-y-6">
                {error && <div className="bg-red-50 text-red-500 text-sm p-3 rounded-lg border border-red-100 animate-shake">{error}</div>}
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      type={showLoginPassword ? "text" : "password"} /* ĐỔI TYPE THEO STATE */
                    ...
                    <button
                      type="button" // Bắt buộc phải có type="button" để không bị submit nhầm form
                ...
                <button type="submit" className="w-full bg-black text-white py-4 rounded-full font-bold hover:bg-gray-800 transition-all shadow-lg active:scale-[0.98]">
                  Đăng nhập
                </button>
              </form>
          ...
          {view === "FORGOT_EMAIL" && (
            <div className="animate-in fade-in slide-in-from-right-4 duration-300 w-full max-w-sm mx-auto">
              <div className="flex items-center justify-between mb-8 border-b border-gray-100 pb-4">
                <h2 className="text-2xl font-extrabold text-gray-900">Khôi phục mật khẩu</h2>
                <button onClick={handleBackToLogin} className="text-[#EF4444] font-bold text-sm hover:opacity-80 transition-opacity">Thoát</button>
              </div>
              ...
              <form onSubmit={handleSendForgotOtp} className="space-y-6">
                ...
                <div>
                  <label className="block text-[11px] font-bold text-gray-500 uppercase mb-2 ml-1 tracking-widest">Email Đăng Ký</label>
                  <input
                    type="email"
          ...
          {view === "FORGOT_RESET" && (
            <div className="animate-in fade-in slide-in-from-right-4 duration-300 w-full max-w-sm mx-auto">
              ...
              <form onSubmit={handleResetPassword} className="space-y-5">
                ...
                <div>
                  <label className="block text-[11px] font-bold text-gray-500 uppercase mb-2 ml-1 tracking-widest">Mã OTP (6 Số Từ Email)</label>
                  <input
                    type="text"
                    maxLength={6}
                ...
                <div>
                  <label className="block text-[11px] font-bold text-gray-500 uppercase mb-2 ml-1 tracking-widest">Mật Khẩu Mới</label>
                  <div className="relative">
                    <input
                      type={showNewPassword ? "text" : "password"} /* ĐỔI TYPE THEO STATE */
        ...
        {/* ================= CỘT BÊN PHẢI (QR CODE - GIỮ NGUYÊN) ================= */}
        <div className="hidden md:flex flex-col items-center justify-center bg-gray-50/50 rounded-[35px] p-12 aspect-square border border-gray-50 relative">
             {/* ... GIỮ NGUYÊN TOÀN BỘ CODE CỘT BÊN PHẢI CỦA BẠN ... */}
             {/* Dưới đây mình giữ nguyên code cũ của bạn cho QR Code để không làm vỡ giao diện */}
             {qrStatus === "EXPIRED" && (
          ...
          {qrStatus === "SCANNED" && (
            <div className="absolute inset-0 bg-white/90 rounded-[35px] z-10 flex flex-col items-center justify-center backdrop-blur-sm shadow-sm transition-all duration-300">
              ...
              <p className="text-gray-900 font-extrabold text-lg">Đã quét thành công</p>
          ...
          <div className="bg-white p-6 rounded-[30px] shadow-xl mb-6 border border-gray-100/50">
            <div className="w-44 h-44 flex items-center justify-center rounded-2xl overflow-hidden relative group">
              {qrToken ? (
                ...
                <div className="text-gray-400 text-xs font-medium animate-pulse">Đang tạo mã...</div>
              )}
            </div>
          </div>
          ...
          <p className="text-gray-400 text-center text-sm max-w-[250px] mb-8 font-medium leading-relaxed">
            Quét mã này bằng ứng dụng <span className="text-gray-900 font-bold">Alo-Chat</span> trên điện thoại của bạn.
          </p>
