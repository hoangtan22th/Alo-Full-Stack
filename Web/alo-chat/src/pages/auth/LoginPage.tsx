...
const LoginPage = () => {
    ...
    const fetchQrToken = async () => {
        try {
            const response = await fetch('http://localhost:8888/api-gateway/auth-service/auth/qr/generate');
            if (response.ok) {
    ...
    useEffect(() => {
        ...
        const interval = setInterval(async () => {
            try {
                const response = await fetch(`http://localhost:8888/api-gateway/auth-service/auth/qr/status/${qrToken}`);
                if (response.ok) {
                    const data = await response.json();
                    setQrStatus(data.status);
    ...
    const handleLogin = async (e: React.FormEvent) => {
        ...
        try {
            const response = await fetch('http://localhost:8888/api-gateway/auth-service/auth/login', {
                method: 'POST',
                ...
                <div className="hidden md:flex flex-col items-center justify-center bg-gray-50/50 rounded-[35px] p-12 aspect-square border border-gray-50 relative">
                    {qrStatus === 'EXPIRED' && (
                        <div className="absolute inset-0 bg-white/80 rounded-[35px] z-10 flex flex-col items-center justify-center backdrop-blur-sm shadow-sm transition-all duration-300">
                            <p className="text-red-500 font-bold mb-4 text-sm bg-red-50 px-4 py-2 rounded-full border border-red-100">Mã QR đã hết hạn</p>
                            <button
                    ...
                    {qrStatus === 'SCANNED' && (
                        <div className="absolute inset-0 bg-white/90 rounded-[35px] z-10 flex flex-col items-center justify-center backdrop-blur-sm shadow-sm transition-all duration-300">
                            ...
                            <p className="text-gray-500 text-sm font-medium tracking-wide mt-1">Vui lòng xác nhận trên điện thoại...</p>
                        </div>
                    )}
