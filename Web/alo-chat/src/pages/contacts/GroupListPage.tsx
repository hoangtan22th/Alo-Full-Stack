...
const getMyGroups = async (userId: string) => {
  try {
    const response = await axiosClient.get("/groups/me");
    // Axios interceptor đã trả về response.data.data
...
const updateApprovalSetting = async (
  ...
  try {
    const response = await axiosClient.patch(`/groups/${groupId}/approval`, {
      isApprovalRequired,
    });
    return { data: response, error: null };
  } catch (error: any) {
...
const updateLinkSetting = async (groupId: string, isLinkEnabled: boolean) => {
  try {
    const response = await axiosClient.patch(`/groups/${groupId}/link`, {
      isLinkEnabled,
    });
    return { data: response, error: null };
  } catch (error: any) {
...
const getFriends = async () => {
  try {
    const response = await axiosClient.get("/contacts/friends");
    return { data: response || [], error: null };
  } catch (error: any) {
...
export default function GroupListPage() {
  ...
  const loadGroups = useCallback(async () => {
    ...
    setError("");
    try {
      const res = await getMyGroups(currentUserId);
      if (res.error) {
        setError(res.error);
        return;
      }
  ...
  const handleLeaveGroup = async () => {
    ...
    try {
      const res = await leaveGroup(selectedGroupId);
      if (res.error) {
