...
import { XMarkIcon, CameraIcon, PhotoIcon, CheckCircleIcon } from "@heroicons/react/24/outline";
import { groupApi } from "@/api/group.api"; // Import từ file trên
...
export default function CreateGroupModal({ onClose, onSuccess }: any) {
  ...
  const handleCreate = async () => {
    ...
    if (selectedUserIds.length < 2) return toast.error("Cần thêm ít nhất 2 bạn bè nữa mới đủ 3 người!");

    try {
      setLoading(true);
      await groupApi.createGroup(groupName, selectedUserIds, avatar || undefined);
      ...
      <div className="bg-white w-full max-w-md rounded-[32px] shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="p-6 overflow-y-auto space-y-6">
          ...
          <div className="flex items-center gap-4">
            <div onClick={() => fileRef.current?.click()} className="w-16 h-16 rounded-2xl bg-gray-100 flex items-center justify-center cursor-pointer border-2 border-dashed relative overflow-hidden">
              {preview ? <img src={preview} className="w-full h-full object-cover" /> : <CameraIcon className="w-6 h-6 text-gray-400"/>}
          ...
          <div className="space-y-3">
            <p className="text-[11px] font-black text-gray-400 uppercase tracking-widest">Chọn thành viên ({selectedUserIds.length})</p>
              ...
              {friends.map(f => (
                ...
                >
                  <div className="flex items-center gap-3">
                    <img src={f.displayAvatar || "/avt-mac-dinh.jpg"} className="w-10 h-10 rounded-full object-cover" />
                    <span className="font-bold text-sm">{f.displayName}</span>
                  </div>
                  {selectedUserIds.includes(f.displayId) && <CheckCircleIcon className="w-6 h-6 text-white"/>}
                </div>
              ))}
            </div>
          </div>
