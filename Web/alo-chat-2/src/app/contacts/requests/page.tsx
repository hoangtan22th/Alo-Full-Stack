...
export default function FriendRequestPage() {
  ...
  const [profileModalFriendshipId, setProfileModalFriendshipId] = useState<string | null>(
    null,
  );
  ...
  const handleAction = async (
    ...
    try {
      if (actionType === "ACCEPT") {
        ...
        const conversation =
          await groupService.createDirectConversation(targetUserId);
        const convoId =
          conversation?._id ||
          conversation?.id ||
    ...
    <div className="flex-1 h-screen bg-[#fafafa] p-4 lg:p-6 overflow-y-auto">
        ...
        <div className="flex items-center gap-3 mb-6 pb-4 border-b border-gray-200">
          ...
          <div>
            ...
            <p className="text-[11px] text-gray-500 font-bold uppercase">
              Bạn có {requests.length} yêu cầu
            </p>
          </div>
        </div>

        {requests.length === 0 ? (
          <div className="text-center py-16 bg-white rounded-[20px] border border-dashed border-gray-200 text-xs italic text-gray-400">
            Trống
          </div>
        ) : (
      ...
      <RequestPreviewModal
        ...
        onViewProfile={() => {
          ...
          setProfileModalFriendshipId(selectedRequest.id);
          setSelectedRequest(null);
        }}
      />
      <FriendProfileModal
        ...
        onClose={() => {
          ...
          setProfileModalFriendshipId(null);
        }}
        userId={profileModalUserId}
        friendshipId={profileModalFriendshipId}
