...
export default function PollMessagePreview({
  pollId,
  isSender,
  onOpenDetails,
    ...
    >
      ...
      <div className="p-5 border-b border-gray-100 flex items-start">
        ...
        <div className="flex-1">
          <h4 className="text-[16px] sm:text-[17px] font-bold leading-snug mb-1 text-gray-900">
            {poll.question}
          </h4>
          {creatorName && (
          ...
          return (
            ...
            >
              <div className="flex items-center z-10 relative">
                {isSelected ? (
                  <div
                    className={`w-5 h-5 sm:w-6 sm:h-6 flex items-center justify-center mr-3 sm:mr-4 bg-blue-600 text-white ${poll.settings.allowMultipleAnswers ? "rounded-md" : "rounded-full"}`}
                  >
                    <CheckIcon className="w-3.5 h-3.5 sm:w-4 sm:h-4" />
                  </div>
                ) : (
                  <div
                    className={`w-5 h-5 sm:w-6 sm:h-6 border-2 mr-3 sm:mr-4 border-gray-300 ${poll.settings.allowMultipleAnswers ? "rounded-md" : "rounded-full"}`}
                  />
                )}
                <span className="flex-1 text-[15px] sm:text-[16px] font-medium truncate text-gray-700">
                  {opt.text}
                </span>
