                ...
                <AnimatePresence mode="wait">
                    {remoteTracks.length > 0 ? (
                        <motion.div
                            ...
                            className={`w-full h-full grid gap-4 ${isMinimized ? 'gap-2' : 'gap-6'} ${remoteTracks.length === 1 ? 'grid-cols-1' :
...
function ControlButton({ active, onClick, icon: Icon }: { active: boolean, onClick: () => void, icon: any }) {
    return (
        <button
            onClick={onClick}
            className={`w-14 h-14 rounded-[1.5rem] flex items-center justify-center transition-all active:scale-90 ${active
